...
class OrderController {
  ...
  async getById(req, res, next) {
    try {
      const o = await service.getById(req.params.id, req.user.tenantId);
  ...
  async create(req, res, next) {
    try {
      ...
      return created(res, await service.create({ type, items, notes, userId: req.user.id, tenantId: req.user.tenantId }));
    } catch (e) { next(e); }
  }

  async confirm(req, res, next) {
    try { return ok(res, await service.confirm(req.params.id, req.user.id, req.user.tenantId)); }
    catch (e) { next(e); }
  }

  async updateStatus(req, res, next) {
    try {
      const o = await service.updateStatus(req.params.id, req.body.status, req.user.tenantId);
      return o ? ok(res, o) : notFound(res, 'Order');
    } catch (e) { next(e); }
  }
